# backend/domain/workflows/market_insight_graph.py
"""
LangGraph 图引擎
...
from domain.services.evidence_pack import build_evidence_pack
from domain.services.session_snapshot import build_memory_snapshot
from infrastructure.tools import ToolCache, ToolGuardrail, ToolRegistry
from infrastructure.exports.report_export import write_html_report
...
class MarketInsightGraphEngine(IGraphEngine):
            ...
            for attempt in range(1, max_attempts + 1):
                ...
                try:
                    ...
                    if self.agent_factory:
                        ...
                        from infrastructure.llm.ark_client import StreamEventType
    ...
    def _execute_debate_exchange(
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                from domain.agents.debate import ChallengerAgent
...
请进行确认（限 100-200 字）："""
    ...
    def _synthesizer_node(self, state: MarketInsightState) -> dict[str, Any]:
        ...
        if self.agent_factory and has_worker_content:
            for attempt in range(1, max_attempts + 1):
                try:
                    from domain.agents.base import AgentContext, AgentOutput
